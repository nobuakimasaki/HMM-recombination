...
parser.add_argument("--prop_out", required=True, help="output JSON of lineage proportions")  # NEW
...
# Filter out lineages with fewer than 30 sequences
lineage_counts = seq_pango_sorted['collapsed'].value_counts()
valid_lineages = lineage_counts[lineage_counts >= 30].index.tolist()

################### This part is saving the json file used to set the initial state probabilities ###################
# Filter counts to valid lineages, then normalise
counts_valid = lineage_counts.loc[valid_lineages]
pi_dict = (counts_valid / counts_valid.sum()).to_dict()      # {"BA.1": 0.42, ...}

# --- write it ---
json_path = pathlib.Path(args.prop_out)
json_path.parent.mkdir(parents=True, exist_ok=True)          # make dirs if needed
with open(json_path, "w") as f:
    json.dump(pi_dict, f, indent=4, sort_keys=True)

print(f"saved initial-probability JSON → {json_path}")
################### This part is saving the json file used to set the initial state probabilities ###################

# Build filtered dictionary with logging
lineages = {}
