...
ggsave("figs/line_plot.png", p_prev, width = 9, height = 6, dpi = 300)
```

```{r}
# Subset points to label: ONS > 5%, complete cases
lab_df <- subset(
  summary_df,
  ons_prop > 0.06 & is.finite(prop) & is.finite(ons_prop) & !is.na(date)
)
lab_df$label_date <- format(lab_df$date, "%Y-%m-%d")
...
p_scatter <- ggplot(summary_df, aes(x = ons_prop, y = prop)) +
  geom_point(alpha = 0.6, size = 1.8, stroke = 0) +
