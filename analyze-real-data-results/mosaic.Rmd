...
gene_labels_df <- gene_bounds %>%
  # drop ORF6–8 from the labels only
  dplyr::filter(!gene %in% c("ORF6", "ORF7a", "ORF7b", "ORF8")) %>%
  mutate(
    parent_pair = factor(last_parent, levels = parent_levels)
  )
...
ggsave("figs/mosaic.png", p_mosaic, height = 10, width = 20)
```
